...
## Pairing with Visual Systems

### Layering Pattern

1. Apply layout structure first (this skill)
2. Add visual styling second (soft-ui-system, etc.)
...
// Step 2: Add visual styling (from soft-ui-system)
<div className="flex h-full overflow-hidden">
  <div className="w-[280px] flex-shrink-0 overflow-hidden border-r border-white/[0.06] bg-soft-dark-100">
    {sidebar}
  </div>
  <div className="flex-1 min-w-0 overflow-y-auto bg-soft-dark-200">
    {content}
  </div>
</div>
```
...
| layout-system | Visual system (soft-ui, etc.) |
|--------------|------------------------------|
| Panel widths (`w-[280px]`) | Background colors (`bg-soft-dark-100`) |
| Flex patterns (`flex-1 min-w-0`) | Border colors (`border-white/[0.06]`) |
